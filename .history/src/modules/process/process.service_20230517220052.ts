...
export class ProcessService {
 ...
 const process = await this.processModel.findByIdAndUpdate(id, updateProcessDto, { new: true }).exec()
  
  if (updateProcessDto.status && updateProcessDto.status !== process.status) {
    process.dateStepUpdate = new Date()
  }
  
  return process    const process = await this.processModel.findByIdAndUpdate(id, {
