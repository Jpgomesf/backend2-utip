...
getProcessesCountByStatus(): { total: number countByStatus: Record<ProcessStatusTypeEnum, number> } {
  const countByStatus: Record<ProcessStatusTypeEnum, number> = {
  ...
  return { total, countByStatus }
}  async getProcessAnalitycs() {
    const processAnalitycs = 
  }
